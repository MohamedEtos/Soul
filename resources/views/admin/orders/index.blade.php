...
@section('css')
    ...
    <link rel="stylesheet" type="text/css"
        href="{{ asset('admin/vendors/css/tables/datatable/extensions/dataTables.checkboxes.css') }}">
    <!-- END: Vendor CSS-->
...
@section('content')
                            ...
                            <div class="btn-group dropdown actions-dropodown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <form id="checkForm" action="{{ route('multideleteOrders') }}" method="post">
                                        @csrf
                                        <button type="submit" form="checkForm" id="checkbox_send"
                                            class="dropdown-item danger" href="#"><i
                            ...
                            <tbody>
                                ...
                                @foreach ($Orderlist as $Order)
                                    <tr class="trRow" data-id_row="{{ $Order->id }}" data-toggle="modal" data-target="#xlarge">
                                        ...
                                        <input type="hidden" class="full_name"
                                            value="{{ $Order->address->full_name ?? 'No Data' }}">
                                        <input type="hidden" class="phone_modeal"
                                            value="{{ $Order->address->phone ?? 'No Data'}}">
                                        <input type="hidden" class="area_modal"
                                            value="{{ $Order->address->area ?? 'No Data'}}">
                                        <input type="hidden" class="floor_number_modal"
                                            value="{{ $Order->address->floor_number ?? 'No Data'}}">
                                        <input type="hidden" class="building_modal"
                                            value="{{ $Order->address->building ?? 'No Data' }}">
                                        <input type="hidden" class="address_modal"
                                            value="{{ $Order->address->address ?? 'No Data'}}">
                                        <input type="hidden" class="status_modal"
                                            value="{{ $Order->payment_status == 'notaccepted' ? 'غير مؤكد' : 'تم ارسال رساله التاكيد' }}">
                                        @php
                                        ...
                                        <td class="product-category productDetalis">
                                            <form id="whatsapp-{{ $Order->id }}" action="{{ route('Send_whatsapp') }}"
                                                method="post">
                                                @csrf
                                            ...
                                            @if($Order->payment_status == 'notaccepted')
                                                <button type="button" class="btn btn-success"
                                                    onclick="sendWhatsAppAndUpdate('{{ $Order->id }}', '{{ $whatsappUrl }}')">إرسال
                                                    رسالة التأكيد</button>
                                            @else
                                                <button type="button" class="btn btn-success"
                                                    onclick="sendWhatsAppAndUpdate('{{ $Order->id }}', '{{ $whatsappUrl }}')">إعادة
                                                    الإرسال</button>
                                            @endif
                                        </td>
                                        ...
                                        <td class="product-category product_name">
                                            @foreach ($Order->items as $item)
                                                {{ $loop->iteration }} - {{ $item->product->name }} ( {{ $item->quantity }} *
                                                {{ $item->price }} ) = {{ $item->quantity * $item->price }} ج.م
                                                <br class="mt-1">
                                        ...
                                        <td class="product-action">
                                            ...
                                            <span class='del text-danger' data-toggle="modal" data-id_del="{{ $Order->id }}"
                                                data-target="#danger">
                                    ...
                                    <div class="data-fields px-2 mt-3">
                                        <div class="row">
                                            ...
                                            <div class="invoice-item border rounded p-2 mt-2">
                                                ...
                                                <div class="col-sm-12 data-field-col">
                                                    <label>السعر</label>
                                                    <input required type="number" name="items[0][price]"
                                                        class="form-control item-price" value="0" min="0" step="0.01">
                                                </div>

                                                <div class="col-sm-12 data-field-col">
                                                    <label>الكميه</label>
                                                    <input required type="number" name="items[0][qty]"
                                                        class="form-control item-qty" value="1" min="0" step="1">
                                                </div>
                                            ...
                                            <div class="col-sm-12 data-field-col">
                                                ...
                                                <input required type="number" name="descount" class="form-control"
                                                    id="descount" value="0" min="0" step="0.01">
                                            </div>
                                            ...
                                            <div class="col-sm-12 data-field-col mt-2">
                                                <small class="">الإجمالي قبل الخصم: <span
                                                        id="subtotalView">0.00</span></small>
                                            </div>
                                            ...
                                            <div class="col-sm-12 data-field-col">
                                                ...
                                                <input required type="text" name='bilding' class="form-control"
                                                    id="bilding">
                                            </div>

                                            <div class="col-sm-12 data-field-col">
                                                <label for="floor_number">الدور </label>
                                                <input required type="text" name='floor_number' class="form-control"
                                                    id="floor_number">
                                            </div>
    ...
    <div class="modal-danger mr-1 mb-1 d-inline-block">
        ...
        <form action="" class="modal fade text-left" method="POST" id="danger" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-1">
                                    <img id="modal_product_image" class=" mt-1 w-100 "
                                        src="{{ asset('store/images/product-13.avif') }}" alt="">
                                </div>
                                <div class="col-5">
